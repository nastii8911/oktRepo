...
const inputField = document.querySelector(".input-field") as HTMLDivElement;
...
function createCard(cardData: Card): HTMLElement {
  const cloned = cardTemplate.cloneNode(true) as HTMLElement;

  const title = cloned.querySelector(".card__title")!;
  title.textContent = cardData.title;

  const img = cloned.querySelector(".card__image") as HTMLImageElement;
  img.src = cardData.imageUrl;
